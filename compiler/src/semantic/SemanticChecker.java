...
public class SemanticChecker extends YaplBaseVisitor<Attributes> {
  ...
  public SemanticChecker(BlockNameExtractor extractor, StandardLibrary stdlib) {
  ...
  public Attributes visitConstDeclaration(ConstDeclarationContext ctx) {
    ...
    final Symbol symbol = new ConstSymbol(name, attrs.type, ctx.literal().getText(), ctx.start);
    symbolTable.addSymbol(symbol);
